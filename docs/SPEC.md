...
| `unique_jumpdest` | Size-adjustable variant of the layout above; matches execution-specs `JochemnetPredeployContractInitcode(code_size=...)`. Takes the optional `code_size:` parameter (default 24576, range `[0x41, 0x01000000]`). The entry jump target (`code_size - 1`) is a **fixed-width** push per size class — `PUSH2` for any `code_size` ≤ 0x010000, `PUSH3` above — not a minimal push. At the 24576 default this is byte-identical to `unique_jumpdest_pre_amsterdam` (same initcode, same CREATE2 addresses), so state already deployed on live testnets stays reachable; crossing the 0x010000 boundary changes the initcode and therefore the derived CREATE2 addresses. |

`code_size:` is rejected on fixed-size patterns (everything except
`unique_jumpdest`) and without `code_pattern:`.
