...
#
# Environment scope: this script is for LARGE CI / dedicated bench hosts
# only (≥100 GB free disk, ≥64 GB RAM, hours of wall time). It does NOT
# run on GitHub-hosted runners — the disk, memory, and time budgets are
# off by orders of magnitude. CI parity for the auto-fill / spec
# pipeline lives in .github/workflows/ci.yml's cross-client-genesis-root
# job, which runs at ~100 MB scale.
#
...
#
# Companion tooling (referenced below; lives on the bench host out of
# tree — not restored here): scripts/verify-bloatnet.sh (RPC verifier),
# scripts/gen-bloatnet-spec/ (YAML spec generator), scripts/engine-driver/
# (CL mock for Engine-API clients).
...
boot_client() {
    ...
    case $client in
        ...
        besu)
            ...
            ;;
        nethermind)
            # Boot recipe is the canonical one from docs/RUNBOOK.md (which
            # itself mirrors client/nethermind/e2e_test.go's
            # nethermindE2EConfigTemplate). Write the boot.cfg next to the DB,
            # then `--config=/data/boot.cfg` — Nethermind reads chainspec
            # path, BaseDbPath, Sync gates, JsonRpc, Merge, etc. from one
            # file. Hand-rolling --Init.X CLI overrides drops fields silently
            # (the original bug — BaseDbPath missing → empty trie on boot).
...
run_one_client() {
    ...
    #
    # MUST happen before start_engine_driver_if_needed — the engine-
    # driver's first action is eth_getBlockByNumber on 127.0.0.1:8545
    # and it exits on first connection-refused. Spawning it before the
    # client's RPC is bound lands the chain at block 0 forever on every
    # Engine-API client (besu + nethermind).
